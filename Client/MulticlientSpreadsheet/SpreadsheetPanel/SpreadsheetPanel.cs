...
namespace SS
{
    ...
    public partial class SpreadsheetPanel : UserControl
    {
        ...
        //Need to see if the selected cell is off the screen 
        ...
        protected override void OnResize(EventArgs eventargs)
        {
            ...
            //drawingPanel.calculateMaxRC(col,row);
            drawingPanel.calculateMaxRC();
        }
        ...
        private class DrawingPanel : Panel
        {
            ...
            public void calculateMaxRC()
            {
                 maxC = (int)Math.Floor(((double)_ssp.Size.Width - LABEL_COL_WIDTH - SCROLLBAR_WIDTH) / DATA_COL_WIDTH);
                 maxR = (int)Math.Floor(((double)_ssp.Size.Height - LABEL_ROW_HEIGHT - SCROLLBAR_WIDTH) / DATA_ROW_HEIGHT);
            }
            ...
            public bool SetSelection(int col, int row)
            {
                ...
                if (col <= 25)
                {
                    if (_selectedCol > (maxC - 1))
                    {
                        _ssp.hScroll.Value = (int)Math.Min((double)_ssp.hScroll.Value + _ssp.hScroll.SmallChange, (double)COL_COUNT);
                        _firstColumn = _ssp.hScroll.Value;
                        maxC++;
                    }
                    else if (_selectedCol < _firstColumn)
                    {
                        _ssp.hScroll.Value = (int)Math.Min((double)_ssp.hScroll.Value - _ssp.hScroll.SmallChange, (double)COL_COUNT);
                        _firstColumn = _ssp.hScroll.Value;
                        maxC--;
                ...
                if (row <= 98)
                {
                    if (_selectedRow > (maxR-1))
                    {
                        _ssp.vScroll.Value = (int)Math.Min((double)_ssp.vScroll.Value + _ssp.vScroll.SmallChange, (double)ROW_COUNT);
                        _firstRow = _ssp.vScroll.Value;
                        maxR++;
                    }
                    else if (_selectedRow < _firstRow)
                    {
                        _ssp.vScroll.Value = (int)Math.Min((double)_ssp.vScroll.Value - _ssp.vScroll.SmallChange, (double)ROW_COUNT);
                        _firstRow = _ssp.vScroll.Value;
                        maxR--;
